...
import pl.fzymek.android.cycloneeye.game.models.Cannon;
import android.util.FloatMath;

public class CameraActivity extends CEGame {
	...
	public class Camera2DScreen extends Screen {
		...
		public Camera2DScreen(CEGame game) {
			super(game);
			...
			for (int i = 0; i < NUM_TARGETS; i++) {
				GameObject target = new GameObject((float) Math.random()
						* WORLD_WIDTH, (float) Math.random() * WORLD_HEIGHT,
						0.5f, 0.5f);
			...
			ballVertices.setVertices(new float[] { -0.1f, -0.1f, 0.1f, -0.1f,
					0.1f, 0.1f, -0.1f, 0.1f }, 0, 8);
			ballVertices.setIndices(new short[] { 0, 1, 2, 2, 3, 0 }, 0, 6);

			targetVertices = new Vertices(glGraphics, 4, 6, false, false);
			targetVertices.setVertices(new float[] { -0.25f, -0.25f, 0.25f,
					-0.25f, 0.25f, 0.25f, -0.25f, 0.25f }, 0, 8);
